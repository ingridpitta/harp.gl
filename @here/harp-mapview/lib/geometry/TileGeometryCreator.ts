...
export class TileGeometryCreator {
     ...
     * @param enabledKinds Optional [[GeometryKindSet]] used to filter objects that should not be
     *      created. A [[Technique]] that does not contain a kind that is in the `enabledKinds` set
     *      will not be created.
     */
    createAllGeometries(
        tile: Tile,
        ...
        const filter = (technique: Technique): boolean => {
            ...
            if (technique.kind instanceof Set) {
                for (const kind of technique.kind as GeometryKindSet) {
        ...
        this.createObjects(tile, decodedTile, enabledKinds !== undefined ? filter : undefined);
    }
