...
import { assert, PerformanceTimer } from "@here/harp-utils";
...
export interface TileGeometryLoader {
    ...
    update(enabledKinds: GeometryKindSet | undefined): void;
    dispose(): void;
}
...
export namespace TileGeometryLoader {
     ...
     * default value.
     *
     * Also gather set of the [[GeometryKind]]s found in the techniques.
     *
     * @export
        ...
        for (const technique of decodedTile.techniques) {
            ...
            if (geometryKind === undefined) {
                if (isFillTechnique(technique)) {
                    ...
                    geometryKind = GeometryKind.Water;
                } else {
            ...
            if (geometryKind instanceof Set) {
                ...
                const geometryKindSet: GeometryKindSet = new Set();
                for (const kind of geometryKind as GeometryKind[]) {
                    foundSet.add(kind);
                    geometryKindSet.add(kind);
                }
                technique.kind = geometryKindSet;
            } else {
...
export class SimpleTileGeometryLoader implements TileGeometryLoader {
    ...
    update(enabledKinds?: GeometryKindSet): void {
     ...
     */
    private prepareForRender(enabledKinds: GeometryKindSet | undefined) {
